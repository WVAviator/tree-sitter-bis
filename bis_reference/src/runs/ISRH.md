# ISRH — Iterative Search

## Overview

Generates Search function masks that can be saved, altered, and reused. With ISRH you can:
...
```
ISRH[*] {report | drawer}[,f,maskname,maskrep]
```
...
| `report` | Required* | Report to process. For more details, see *Specifying Reports or Drawers to Process*. |
| `drawer` | Required* | Drawer to process. Use to process all reports in a given drawer. For more details, see *Specifying Reports or Drawers to Process*. |
...
*Specify either `report` or `drawer`, not both.
